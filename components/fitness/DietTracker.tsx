...
import { ChevronLeft, ChevronRight, AlertTriangle, CheckCircle, Bot } from 'lucide-react'
...
const EMPTY_FORM: DietForm = {
  calories: '', protein_g: '', carbs_g: '', fat_g: '', water_l: '', memo: '',
  breakfast: '', lunch: '', dinner: '', snack: '',
}

function formFromRecord(r: FitnessDiet): DietForm {
  return {
...
export default function DietTracker() {
  ...
  const [form, setForm] = useState<DietForm>(EMPTY_FORM)
  ...
  const loadDate = useCallback(async (date: string) => {
    ...
    try {
      ...
      if (record) {
        ...
        setHasAiMemo(!!record.memo && record.memo.length > 10)
      } else {
        setForm(EMPTY_FORM)
        setHasAiMemo(false)
      }
  ...
  const hasMacroData = caloriesVal > 0 || proteinVal > 0
  const hasMealData  = MEAL_TYPES.some(m => form[m.key])
    ...
    <div className="space-y-4">
      ...
      <div className="md:grid md:grid-cols-2 md:gap-5 space-y-4 md:space-y-0">
        ...
        <div className="space-y-4">
          ...
          <section className="bg-white rounded-2xl border border-gray-100 p-4 space-y-1">
            <h3 className="font-bold text-gray-900 text-sm mb-3">
              오늘의 목표
              {!hasMacroData && (
                <span className="ml-2 text-xs font-normal text-gray-400">AI 코치 탭에서 생성하거나 직접 입력하세요</span>
              )}
            </h3>
            {isLoading ? (
              ...
              <>
                ...
                {form.memo && (
                  <div className="pt-2 pb-1">
                    <p className="text-[11px] text-purple-500 font-medium mb-1 flex items-center gap-1">
                      <Bot size={11} />AI 추천 식단
                    </p>
                    <p className="text-xs text-gray-500 bg-purple-50 rounded-xl px-3 py-2 leading-relaxed whitespace-pre-line">
                      {form.memo}
                    </p>
                  </div>
                )}
        ...
        <div className="space-y-4">
          ...
          {/* 식사 기록 입력 */}
          <section className="bg-white rounded-2xl border border-gray-100 p-4 space-y-3">
            <h3 className="font-bold text-gray-900 text-sm">식사 기록</h3>
            {isLoading ? (
              <p className="text-xs text-gray-400 text-center py-4">불러오는 중...</p>
            ) : (
              <>
                  ...
                  <tbody>
                    {history.map(d => (
                      <tr
                        ...
                        className={`border-b border-gray-50 last:border-0 cursor-pointer ${d.date === selectedDate ? 'bg-blue-50' : 'hover:bg-gray-50'}`}
                      >
